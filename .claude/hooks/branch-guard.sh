...
# Enforces the worktree rule in AGENTS.md.
...
case "$branch" in
  main|master)
    deny "Workflow violation: the worktree owning this file is on branch \"$branch\". No changes on main — every change goes through a branch + PR. Create a fresh topic worktree: .claude/scripts/worktree.sh new <topic>. See AGENTS.md."
    ;;
esac
